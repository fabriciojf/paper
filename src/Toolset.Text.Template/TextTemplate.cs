...
namespace Toolset.Text.Template
{
  ...
  public class TextTemplate
  {
    ...
    private TextTemplate(Expression expression)
    {
      this.expression = expression;
    }

    /// <summary>
    /// Analise o template e constrói uma instância de TextTemplate apropriada.
    /// </summary>
    /// <param name="template">Um template do TextTemplate.</param>
    /// <returns>A instância do template.</returns>
    public static TextTemplate Parse(string template)
    {
      return Parse(template, ParseOptions.None);
    }
    ...
    /// </summary>
    /// <param name="template">Um template do TextTemplate.</param>
    /// <param name="options">Opções de manipulação do parser.</param>
    /// <returns>A instância do template.</returns>
    public static TextTemplate Parse(string template, ParseOptions options)
    {
      var expression = ExpressionParser.Parse(template, options);
      var instance = new TextTemplate(expression);
      return instance;
    }
    
    /// <summary>
    /// Aplica o template com os objetos de contexto indicados produzindo
    /// o texto formatado.
    /// 
    /// Em geral o método é usado de duas formas:
    /// 
    /// 1.  Quando o template é aplicado sobre um objeto apenas o parâmetro
    ///     "target" pode ser informado, o parâmetro "context" pode ser nulo.
    ///     Isto porque o objeto geralmente contém toda informação relevante
    ///     para a aplicação do template.
    ///     
    ///     Exemplo:
    ///     
    ///       var obj = new object {
    ///         ... propriedades ...
    ///       }
    ///       var template = TextTemplate.Create(...)
    ///       template.Apply(obj)
    ///       
    /// 2.  Quando o template é aplicado sobre um XML o primeiro parâmetro
    ///     "target" contém uma instância de `XElement` representando o XML
    ///     e o parâmetro "context" pode, opcionalmente, ser informado para
    ///     adicionar propriedades não presentes no XML.
    ///     
    ///     Exemplo:
    ///     
    ///       var xml = "...".ToXElement();
    ...
    ///       }
    ///       var template = TextTemplate.Create(...)
    ///       template.Apply(xml, ctx)
    /// </summary>
    /// <param name="target">
    /// O objeto principal da manipulação do texto.
    /// Pode ser um objeto ou uma instância de XML do tipo XElement.
    /// 
    /// Quando usando XML como objeto principal o parâmetro "context" pode
    /// ser usado para adicionar propriedades adicionais não presentes no 
    /// XML.
    /// </param>
    /// <param name="context">
    /// Um objeto de contexto adicional.
    /// O contexto adicional é acessado pelo template via "$ctx.".
    /// 
    /// Em geral, quando aplicando o template em cima de um XML, é
    /// conveniente anexar um contexto adicional contendo informações
    /// não presentes no XML.
    /// </param>
    /// <returns></returns>
    public string Apply(object target, object context = null)
    {
      var result = this.expression.Compute(target, context);
      return result;
    }
  }
}
