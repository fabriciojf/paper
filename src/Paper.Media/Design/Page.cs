...
namespace Paper.Media.Design
{
  public class Page
  {
      ...
      if (queryString != null)
      {
        ...
        foreach (var arg in args)
        {
          switch (arg.name)
          {
            case "page":
              {
                if (int.TryParse(arg.value, out int number))
                {
                  Number = number;
                }
                break;
    ...
    {
      if (IsLimitSet)
        yield return new KeyValuePair<string, int>("limit", Limit);
      else
        yield return new KeyValuePair<string, int>("pageSize", Size);

      if (IsOffsetSet)
        yield return new KeyValuePair<string, int>("offset", Offset);
      else
        yield return new KeyValuePair<string, int>("page", Number);
    }
