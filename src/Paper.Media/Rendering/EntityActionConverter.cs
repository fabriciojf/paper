...
namespace Paper.Media.Rendering
{
  ...
  {
    public static EntityAction ConvertToEntityAction(object graph, IContext context)
    {
      ...
      if (action == null)
      {
        ...
        else
        {
          ...
          {
            var field = CreateField(graph, key, context);
    ...
    private static Field CreateField(object graph, string key, IContext context)
    {
      var field = new Field();
      ...
      {
        attribute.RenderField(field, property, graph);
      }
